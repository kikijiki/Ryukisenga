...
public class TouchInterceptor extends ListView {
    private static final int DRAG_ALPHA = 180;
    ...
    private WeakReference<ImageView> mTrashcan;
    private boolean mOverTrash = false;
    private WeakReference<View> mFooter = null;
    private int mDragWidth = 64;
    ...
    public TouchInterceptor(Context context, AttributeSet attrs) {
        ...
        mDragWidth = res.getDimensionPixelSize(R.dimen.drag_width);
    }
    ...
    private void dragView(int x, int y) {
        mWindowParams.alpha = DRAG_ALPHA;

        mWindowParams.x = x - mDragPointX + mXOffset;

        mWindowParams.y = Math.max(mYOffset - mItemHeightNormal, y - mDragPointY + mYOffset);

        mWindowManager.updateViewLayout(mDragView, mWindowParams);

        if (mTrashcan != null) {
            int trashCoords[] = {0,0};
            mTrashcan.get().getLocationOnScreen(trashCoords);
            int tr = trashCoords[0] + mTrashcan.get().getWidth();
            int tb = trashCoords[1] + mTrashcan.get().getWidth();

            if (y + mYOffset < tb && x < tr) {
                mTrashcan.get().getDrawable().setLevel(1);
                mOverTrash = true;
            } else {
                mTrashcan.get().getDrawable().setLevel(0);
                mOverTrash = false;
            }
        }
    }
    ...
    public void setTrashcan(ImageView trash) {
        mTrashcan = new WeakReference<ImageView>(trash);
    }
    ...
    final public void addFooterView(View v, Object data, boolean isSelectable) {
        AbsListView.LayoutParams p = new AbsListView.LayoutParams(AbsListView.LayoutParams.MATCH_PARENT, mItemHeightNormal);
        v.setLayoutParams(p);
        super.addFooterView(v, data, isSelectable);
        mFooter = new WeakReference<View>(v);
    }
